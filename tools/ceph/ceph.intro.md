...
- RADOS Gateway(RGW)
  - Ceph 분산 스토리지 클러스터 위에 구축된 RESTful 오브젝트 스토리지 인터페이스로, Amazon S3 및 OpenStack Swift와 호환되는 API를 제공
  - 대규모 데이터 저장, 높은 가용성, 강한 일관성을 제공하며, 웹 기반 애플리케이션에서 직접 데이터를 저장하고 관리할 수 있는 고성능 게이트웨이 역할
  - Object Storage 사용시 별도로 활성화 필요

- Metadata Servers(MDS)
  - Ceph file system을 사용할 때만 필요한 구성요소로 파일 이름, 접근 권한, 소유자 등의 메타 데이터를 저장한다.
  - CephFS 사용시 별도로 활성화 필요

- 각 서비스 활성화(enable) 방법
  - MON/MGR : ceph 초기화(init)시 자동 활성화
  - RGW : Object Storage 사용시 별도 활성화
  - MDS : CephFS 사용시 별도 활성화
