...
- `jackson-bom`을 참조하는 경우 sub module 들은 version을 직접 입력할 필요가 없음
...
- spring-boot 프로젝트의 경우 spring-boot가 내부적으로 bom을 적용하기 때문에 별도의 버전 명시 필요하지 않음
- spring-boot는 `jackson.core`를 기본적으로 로딩 하므로 별도로 명시 할 필요 없음
