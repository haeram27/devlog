...
`@JsonTypeInfo`는 Json의 특정 필드 값 또는 클래스 이름을 기준으로 맵핑될 실제 클래스를 `JsonSubTypes`로 명시해 주는 기법이다. `@JsonSubTypes`는 특정 기준이 매칭되었을때, 실제로 Json이 맵핑되어야 하는 클래스를  
...
@JsonSubTypes(
    JsonSubTypes.Type(value = ReportScheduleUnspecifiedDto::class, name = "SCHEDULE_TYPE_UNSPECIFIED"),
...
@JsonNaming(PropertyNamingStrategies.SnakeCaseStrategy::class)
data class ReportScheduleUnspecifiedDto(
    val timezone: String = "",
    override val type: ReportScheduleType = ReportScheduleType.SCHEDULE_TYPE_UNSPECIFIED,
