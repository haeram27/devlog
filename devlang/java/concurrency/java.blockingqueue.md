...
```java
    ...
    public static ExecutorService customGoodUnboundedCachedThreadPool() {
        ...
        ThreadPoolExecutor executor = new ThreadPoolExecutor(
                    halfNumberOfThreads, // minimum thread pool size
                    maxNumberOfThreads, // maximum thread pool size
        ...
        // If thread pool size to be 0, thread will be created again by adding task
...
- `LinkedBlockingQueue`: 가변 크기 큐(연결 리스트 기반). 용량을 제한할 수도 있고(bounded), 제한 없이(unbounded) 사용할 수도 있습니다. 대개 ArrayBlockingQueue보다 높은 처리량(throughput)을 보입니다. 일반적으로 Unbounded로 가장 많이 사용됨
