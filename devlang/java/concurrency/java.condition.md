...
만약 lock.lock()을 호출하지 않은 상태에서 condition.await()나 signal()을 호출하면, 자바에서는 실행 시점에 IllegalMonitorStateException이 발생합니다.
...
class Buffer {
    ...
    public void produce(String data) throws InterruptedException {
        ...
        try {
            while (!empty) {
                notFull.await(); // 3. 버퍼가 꽉 차면 대기
            }
            this.data = data;
            empty = false;
            System.out.println("Produced: " + data);
            notEmpty.signal(); // 4. 비어있지 않다고 알림
        } finally {
            lock.unlock(); // 5. Lock 해제
        }
    }
