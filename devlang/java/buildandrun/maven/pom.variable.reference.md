...
Maven [POM](https://maven.apache.org/guides/introduction/introduction-to-the-pom.html)(Project Object Model)은 java 프로젝트를 빌드하기 위한 maven의 설정 내용을 정형화한한 형식의 xml 문서로 정의한 것을 말한다. 또한 POM(`.pom`) 파일은 Java 프로젝트의 라이브러리 의존성 정보를 포함하므로 BOM(`Bill Of Materials`)의 역할로 중요하게 사용되고 있다.
...
## **환경 변수**
    ...
    ${env.HOME}          <!-- 사용자 홈 디렉터리 -->
    ${env.PATH}          <!-- 시스템 PATH -->
    ${env.MY_SETTING}    <!-- OS 환경 변수 MY_S ETTING -->
    ```
