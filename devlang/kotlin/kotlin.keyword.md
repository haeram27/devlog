...
- field: 속성(property)의 값을 실제로 저장하는 '숨겨진 변수'를 가리키며, 커스텀 getter/setter 안에서만 사용 가능합니다.
- get: 속성값을 읽을 때 실행되는 로직을 정의하는 커스텀 getter 선언 키워드입니다.
- set: 속성값을 저장할 때 실행되는 로직을 정의하는 커스텀 setter 선언 키워드입니다.
- it: 람다식에서 인수가 하나뿐일 때, 그 인수를 가리키는 기본 이름입니다.
- file: @file:JvmName처럼 어노테이션이 클래스가 아닌 파일 전체에 적용됨을 나타내는 타겟 키워드입니다.
- import: 다른 패키지에 있는 클래스, 함수, 속성을 현재 파일에서 사용할 수 있도록 불러옵니다.
- where: 제네릭 타입 파라미터에 대해 여러 개의 제약 조건(상한 경계)을 동시에 걸 때 사용합니다.
- property: 어노테이션의 타겟을 속성(field + getter/setter 전체)으로 지정할 때 사용하는 키워드입니다.
- receiver: 확장 함수나 리터럴에서 해당 함수가 호출되는 대상 객체를 어노테이션 타겟으로 지정할 때 사용합니다.
- param: 생성자의 파라미터에 어노테이션을 적용하고 싶을 때 사용하는 타겟 키워드입니다.
- setparam: setter의 파라미터(value)에 어노테이션을 적용할 때 사용하는 타겟 키워드입니다.
- delegate: 위임된 속성(delegated property)의 위임 객체(by 뒤의 객체)를 어노테이션 타겟으로 지정할 때 사용합니다.
field: `var name = "" set(v) { field = v.trim() }` (실제 저장 공간에 접근)
get: `val isEmpty get() = size == 0` (읽기 로직 정의)
set: `var score = 0 set(v) { if(v >= 0) field = v }` (쓰기 로직 정의)
it: `list.filter { it > 10 }` (단일 파라미터 참조)
file: `@file:JvmName("Utils")` (파일 전체에 어노테이션 적용)
import: `import kotlin.math.PI` (외부 요소 가져오기)
where: `fun <T> copy(s: T) where T : CharSequence, T : Comparable<T>` (다중 제약)
property: `@property:Id val userId: String` (속성 전체에 어노테이션 적용)
receiver: `fun @receiver:Fancy String.myFunc() {}` (확장 대상에 어노테이션 적용)
param: `class User(@param:Ann val name: String)` (생성자 파라미터에 적용)
setparam: `var age: Int = 0 set(@setparam:Ann value) { field = value }` (setter 인자에 적용)
delegate: `@delegate:Transient val memo by lazy { "..." }` (위임 객체에 적용)
