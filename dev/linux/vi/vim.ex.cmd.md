...
| 명령어 | 전체 형식/예시 | 설명 |
| --- | -------- | -- |
| `:s`     | `:s/foo/bar/g`        | 치환(substitute 별칭). 현재 줄에서 문자열 치환 |
| `:%s`    | `:%s/foo/bar/gc`      | 전체 파일에서 치환 (g=모두, c=확인) |
| `:g`     | `:g/pattern/d`        | 매칭 조건 실행. 패턴 포함 줄에 명령 적용 |
| `:v`     | `:v/pattern/d`        | 비매칭 조건 실행. 패턴 비 포함 줄에 명령 적용 |
| `:y`     | `:3,5y`               | 줄복사(yank 별칭). 3~5줄 복사 |
| `:p`     | `:put`                | 붙여넣기(put, p). 마지막으로 삭제 또는 복사된 내용을 현재 줄 뒤에 붙여넣기 |
| `:d`     | `:10,15d`             | 줄삭제. 10~15줄 삭제 |
| `:t`     | `:3t$`                | 줄복사(copy, co 별칭)하여 붙이기. 3번 줄을 마지막 줄($) 뒤로 복사 |
| `:m`     | `:5m0`                | 줄이동(move 별칭). 5번 줄을 맨 앞(0)으로 이동 |
| `:r`     | `:r file.txt`         | 외부파일 삽입. 파일 내용 현재 줄 아래에 삽입 |
| `:sort`  | `:%sort`              | 지정된 줄(범위)을 알파벳 순으로 정렬. `sort !` 역순 정렬. `sort u` 중복제거 후 정렬. `sort n` 숫자기준 정렬|
| `:normal {commands}` | `:norm a3` | EX 모드에서 normal모드 명령실행(normal, norm). 커서 다음에 3입력|
...
| 명령어 | 전체 형식/예시 | 설명 |
| --- | -------- | -- |
| `:ls` / `:buffers` | `:ls`              | 열려 있는 버퍼 목록 보기(ls, buffers, files) |
